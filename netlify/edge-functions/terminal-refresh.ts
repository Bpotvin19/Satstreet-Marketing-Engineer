export default async function terminalRefresh(_request: Request, context: any) {
  ...
  const js = `<script data-terminal-refresh>(function(){function run(){document.querySelectorAll('.avatar').forEach(x=>x.remove());var g=document.getElementById('greeting');if(g){var h=new Date().getHours();g.textContent='Good '+(h<12?'morning':h<17?'afternoon':'evening')+'.'}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run()})();</script>`;
